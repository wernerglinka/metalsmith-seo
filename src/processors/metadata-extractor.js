...
export function extractMetadata(filePath, frontmatter, options) {
  const { hostname, seoProperty = 'seo', defaults = {}, fallbacks = {}, wordsPerMinute = 200 } = options;
...
function extractTitle(seoData, frontmatter, defaults, fallbacks) {
  const extracted = extractWithPriority(seoData, frontmatter, 'title', fallbacks.title || 'title');
...
function extractDescription(seoData, frontmatter, defaults, fallbacks) {
  ...
  const extracted = extractWithPriority(seoData, frontmatter, {
    seo: 'description',
...
function extractPublishDate(seoData, frontmatter, fallbacks) {
  const date = extractWithPriority(seoData, frontmatter, {
    seo: 'publishDate',
...
function extractAuthor(seoData, frontmatter, fallbacks, defaults) {
  ...
  let author = extractWithPriority(seoData, frontmatter, 'author', fallbacks.author || 'author.name');
...
function extractKeywords(seoData, frontmatter, fallbacks) {
  const keywords = extractWithPriority(seoData, frontmatter, {
    seo: 'keywords',
