...
import { generateTwitterCardTags, twitterCardTagsToHtml } from '../../src/generators/twitter-generator.js';

describe('Twitter Generator Branch Coverage Tests', () => {
  beforeEach(() => {
    process.env.NODE_ENV = 'test';
  });

  describe('generateTwitterCardTags branches', () => {
    it('should handle all card type determination branches', () => {
      ...
      const siteConfigResult = generateTwitterCardTags(
        { title: 'Test' },
        {
    ...
    it('should handle core Twitter tags with all optional fields', () => {
      const metadata = {
        title: 'Very Long Title That Should Be Truncated Because Twitter Has Limits on Title Length',
        description:
          'Very long description that should be truncated because Twitter has specific limits on description length that we need to respect. This description is intentionally long to test the truncation functionality properly.',
    ...
    it('should handle core Twitter tags with missing optional fields', () => {
      const metadata = {
        title: 'Test Title'
    ...
    it('should handle creator fallback hierarchy', () => {
    ...
    it('should skip metadata.author when it is a real name (contains spaces)', () => {
    ...
    it('should skip metadata.author that is not a valid handle and emit no creator if no other fallback', () => {
      const result = generateTwitterCardTags(
        {
          title: 'Test',
          author: 'Werner Glinka'
    ...
    it('should accept metadata.author that already looks like a handle', () => {
      const result = generateTwitterCardTags(
        {
          title: 'Test',
    ...
    it('should handle Twitter handle formatting edge cases', () => {
      const testCases = [
      ...
      testCases.forEach(({ input, expected }) => {
        ...
        const creatorTag = result.metaTags.find((tag) => tag.name === 'twitter:creator');
        assert(
          creatorTag && creatorTag.content === expected,
    ...
    it('should handle title truncation edge cases', () => {
    ...
    it('should handle description truncation edge cases', () => {
      ...
      const sentenceDesc =
        'This is the first sentence that needs to be long enough to trigger truncation behavior when combined with other sentences. This is the second sentence that definitely makes it too long and should be removed during the truncation process for Twitter cards and social media optimization.';
      ...
      assert(
        !sentenceDescTag.content.includes('second sentence'),
        'Should truncate before sentence that makes it too long'
      );
    ...
    it('should handle summary_large_image card type branches', () => {
      // Test with image
    ...
    it('should handle summary card type branches', () => {
      // Test with image
      const withImageResult = generateTwitterCardTags(
        {
          twitterCard: 'summary',
    ...
    it('should handle app card type branches', () => {
      ...
      assert(
        fullAppResult.metaTags.find((tag) => tag.name === 'twitter:app:id:iphone'),
        'Should have iOS app ID'
      );
      assert(
        fullAppResult.metaTags.find((tag) => tag.name === 'twitter:app:url:iphone'),
        'Should have iOS app URL'
      );
      assert(
        fullAppResult.metaTags.find((tag) => tag.name === 'twitter:app:id:googleplay'),
        'Should have Android app ID'
      );
      assert(
        fullAppResult.metaTags.find((tag) => tag.name === 'twitter:app:url:googleplay'),
        'Should have Android app URL'
      );
      assert(
        fullAppResult.metaTags.filter((tag) => tag.name === 'twitter:app:name:iphone').length === 1,
        'Should have iOS app name'
      );
      assert(
        fullAppResult.metaTags.filter((tag) => tag.name === 'twitter:app:name:googleplay').length === 1,
        'Should have Android app name'
      );
      ...
      assert(
        !minimalAppResult.metaTags.find((tag) => tag.name === 'twitter:app:id:iphone'),
        'Should not have iOS tags when missing'
      );
      assert(
        !minimalAppResult.metaTags.find((tag) => tag.name === 'twitter:app:id:googleplay'),
        'Should not have Android tags when missing'
      );
      assert(
        !minimalAppResult.metaTags.find((tag) => tag.name === 'twitter:app:name:iphone'),
        'Should not have app name tags when missing'
      );
    });

    it('should handle player card type branches', () => {
      ...
      assert(
        !minimalVideoResult.metaTags.find((tag) => tag.name === 'twitter:player'),
        'Should not have player when no URL'
      );
      assert(
        !minimalVideoResult.metaTags.find((tag) => tag.name === 'twitter:player:width'),
        'Should not have width when missing'
      );
      assert(
        !minimalVideoResult.metaTags.find((tag) => tag.name === 'twitter:player:height'),
        'Should not have height when missing'
      );
      assert(
        !minimalVideoResult.metaTags.find((tag) => tag.name === 'twitter:player:stream'),
        'Should not have stream when missing'
      );
      assert(
        !minimalVideoResult.metaTags.find((tag) => tag.name === 'twitter:image'),
        'Should not have image when missing'
      );
    });

    it('should handle default case for unknown card types', () => {
  ...
  describe('twitterCardTagsToHtml branches', () => {
    it('should handle HTML escaping in Twitter tags', () => {
      const metaTags = [
        {
      ...
      assert(
        html.includes('content="Title with &quot;quotes&quot; &amp; &lt;tags&gt;"'),
        'Should escape content attribute'
      );
      assert(
        html.includes('content="Description with &#39;single quotes&#39; and &amp;amp; entities"'),
        'Should escape single quotes and existing entities'
      );
    });

    it('should handle non-string values in tags', () => {
      const metaTags = [
