...
import { generateMetaTags, metaTagsToHtml, linkTagsToHtml } from '../../src/generators/meta-generator.js';

describe('Meta Generator Branch Coverage Tests', () => {
  beforeEach(() => {
    process.env.NODE_ENV = 'test';
  });

  describe('generateMetaTags branches', () => {
    it('should handle metadata without description', () => {
      ...
      assert(
        result.metaTags.find((tag) => tag.name === 'author'),
        'Should have author tag'
      );
    });

    it('should handle keywords as string vs array', () => {
    ...
    it('should handle all content types for robots directive', () => {
      // Test article type
    ...
    it('should handle noIndex metadata', () => {
      const metadata = {
        title: 'Test',
        noIndex: true,
        type: 'article'
    ...
    it('should handle custom robots metadata', () => {
      const metadata = {
        title: 'Test',
    ...
    it('should handle all optional site config options', () => {
      ...
      assert(
        result.metaTags.find((tag) => tag.name === 'theme-color'),
        'Should have theme-color'
      );
      assert(
        result.metaTags.find((tag) => tag.httpEquiv === 'content-language'),
        'Should have content-language'
      );
      assert(
        result.metaTags.find((tag) => tag.name === 'publisher'),
        'Should have publisher'
      );
      assert(
        result.metaTags.find((tag) => tag.name === 'copyright'),
    ...
    it('should handle partial googlebot configurations', () => {
      const metadata = { title: 'Test' };
    ...
    it('should handle article-specific metadata branches', () => {
      const metadata = {
        title: 'Article',
        type: 'article',
      ...
      assert(
        result.metaTags.find((tag) => tag.name === 'article:published_time'),
        'Should have published time'
      );
      assert(
        result.metaTags.find((tag) => tag.name === 'article:modified_time'),
        'Should have modified time'
      );
      assert(
        result.metaTags.find((tag) => tag.name === 'article:author'),
        'Should have article author'
      );
    ...
    it('should handle article metadata without optional fields', () => {
      const metadata = {
        title: 'Article',
        type: 'article'
    ...
    it('should handle metadata without canonical URL', () => {
      const metadata = {
        title: 'Test'
        // No canonicalURL
      };
    ...
    it('should handle metadata without author', () => {
      const metadata = {
        title: 'Test'
        // No author
      };
    ...
    it('should handle keywords falsy values', () => {
      // Test null keywords
    ...
    it('should handle robots directive returning null', () => {
    ...
    it('should test noIndex false path', () => {
      const metadata = {
        title: 'Test',
        noIndex: false,
    ...
    it('should test robots metadata false path', () => {
      const metadata = {
        title: 'Test',
    ...
    it('should handle missing individual site config values', () => {
    ...
    it('should handle individual Googlebot directive presence/absence', () => {
      ...
      const twoDirectivesResult = generateMetaTags(
        { title: 'Test' },
        {
          maxSnippet: 150,
    ...
    it('should handle article with missing individual fields', () => {
  ...
  describe('metaTagsToHtml branches', () => {
    it('should handle meta tags with all attribute types', () => {
      const metaTags = [
    ...
    it('should handle meta tags with missing specific attributes', () => {
      // Test missing name
    ...
    it('should handle meta tags missing content', () => {
      const metaTags = [
        { name: 'test' }
        // No content
      ];
    ...
    it('should handle HTML escaping in meta tags', () => {
      const metaTags = [{ name: 'test<script>', content: 'Value with "quotes" & <tags>' }];

      const html = metaTagsToHtml(metaTags);
      assert(html.includes('name="test&lt;script&gt;"'), 'Should escape name attribute');
      assert(
        html.includes('content="Value with &quot;quotes&quot; &amp; &lt;tags&gt;"'),
        'Should escape content attribute'
      );
    });
  });

  describe('linkTagsToHtml branches', () => {
    it('should handle link tags with all standard attributes', () => {
      const linkTags = [
    ...
    it('should handle link tags with missing standard attributes', () => {
      // Test missing rel
    ...
    it('should handle link tags with custom attributes beyond standard ones', () => {
      const linkTags = [
        {
          rel: 'stylesheet',
          href: '/style.css',
    ...
    it('should not duplicate standard attributes in custom attributes loop', () => {
      const linkTags = [
        {
          rel: 'canonical',
    ...
    it('should handle link tags with only custom attributes', () => {
      const linkTags = [
    ...
    it('should handle HTML escaping in link tags', () => {
      const linkTags = [
        {
          rel: 'test<script>',
      ...
      assert(
        html.includes('href="https://example.com?param=&quot;value&quot;&amp;other=&lt;test&gt;"'),
        'Should escape href attribute'
      );
      assert(
        html.includes('custom="Value with &quot;quotes&quot; &amp; &lt;tags&gt;"'),
        'Should escape custom attributes'
      );
    });
  });

  describe('escapeHtml function branches', () => {
    ...
    it('should handle zero as falsy content', () => {
      // Zero is falsy so it won't get a content attribute
      const metaTags = [{ name: 'zero-test', content: 0 }];
    ...
    it('should handle falsy content values', () => {
    ...
    it('should handle escaping edge cases', () => {
      // Test all special characters together
      const metaTags = [{ name: 'all-chars', content: '&<>"\'test' }];
    ...
    it('should handle empty string inputs', () => {
      const metaTags = [{ name: 'empty-content', content: '' }];
