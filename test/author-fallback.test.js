...
describe('Author fallback integration', () => {
  it('should use siteOwner from site metadata when no author is specified', (_t, done) => {
    ...
    plugin(files, metalsmith, (err) => {
      ...
      assert.strictEqual(seoMetadata.author, 'Werner Glinka', 'seoMetadata should contain siteOwner as author');

      // Test HTML generation as secondary check
      assert(html.includes('Werner Glinka'), 'Should include siteOwner in generated HTML');

      done();
    });
  });

  it('should prioritize frontmatter author over siteOwner', (_t, done) => {
    const metalsmith = Metalsmith('test/fixtures/html').metadata({
      site: {
        url: 'https://example.com',
        title: 'Test Site',
        siteOwner: 'Werner Glinka'
    ...
    plugin(files, metalsmith, (err) => {
      ...
      assert(
        html.includes('<meta name="author" content="Jane Doe"'),
        'Should use specific author, not siteOwner fallback'
      );

      assert(html.includes('"name": "Jane Doe"'), 'Should use specific author in JSON-LD');

      // Should not contain the siteOwner
      assert(!html.includes('Werner Glinka'), 'Should not include siteOwner when specific author is provided');

      done();
    });
  });

  it('should use card author when available', (_t, done) => {
    const metalsmith = Metalsmith('test/fixtures/html').metadata({
      site: {
        url: 'https://example.com',
        title: 'Test Site',
    ...
    plugin(files, metalsmith, (err) => {
      ...
      assert(
        html.includes('<meta name="author" content="Marie Curie"'),
        'Should use card author over siteOwner fallback'
      );

      assert(html.includes('"name": "Marie Curie"'), 'Should use card author in JSON-LD');

      done();
    });
  });

  it('should handle array authors from card and join them', (_t, done) => {
    const metalsmith = Metalsmith('test/fixtures/html').metadata({
      site: {
        url: 'https://example.com',
        title: 'Test Site',
    ...
    plugin(files, metalsmith, (err) => {
      ...
      assert(
        html.includes('<meta name="author" content="Albert Einstein, Isaac Newton"'),
        'Should join array authors with comma'
      );

      assert(html.includes('"name": "Albert Einstein, Isaac Newton"'), 'Should join array authors in JSON-LD');

      done();
    });
  });

  it('should work with plugin defaults when no site metadata', (_t, done) => {
    const metalsmith = Metalsmith('test/fixtures/html');
    ...
    plugin(files, metalsmith, (err) => {
      ...
      assert(
        html.includes('<meta name="author" content="Plugin Default Author"'),
