...
describe('SiteOwner Fallback (Isolated)', () => {
  ...
  it('should use siteOwner when no author exists in frontmatter', (_t, done) => {
    ...
    plugin(testFiles, metalsmith, (err) => {
      ...
      try {
        ...
        assert.strictEqual(seoMetadata.author, 'Isolated Test Author', 'Author should be extracted from siteOwner');

        // Secondary assertion: Check HTML output
        assert(html.includes('Isolated Test Author'), 'HTML should contain the siteOwner name');
        ...
        assert(html.includes('"name": "Isolated Test Author"'), 'JSON-LD should include author name');
  ...
  it('should work with minimal configuration', (_t, done) => {
  ...
  it('should handle missing siteOwner gracefully', (_t, done) => {
    ...
    plugin(testFiles, metalsmith, (err) => {
      ...
      assert.strictEqual(seoMetadata.author, undefined, 'Author should be undefined when no siteOwner provided');

      // Should not have author meta tag
      assert(!html.includes('<meta name="author"'), 'Should not generate author meta tag when no author available');
